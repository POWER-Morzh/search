...
function setNumber_module(valoare)
	{
		...
		document.getElementById('number_items_module').value = valoare;
		document.forms['searchFormModule'].submit();
	}

function setPage_module(valoare)
	{
		if (valoare==0 || valoare.length==0) valoare = 1;
		document.getElementById('current_page_module').value = valoare;
		document.getElementById('number_items_module').value = document.getElementById('limit1').value;
		document.forms['searchFormModule'].submit();
	}
function save_selected_users_module()
	{
		...
		var lista = '', primul = 0;
		for (i=0; i<document.searchFormModule.cbuserlistaModule.length; i++)
			if (document.searchFormModule.cbuserlistaModule[i].checked)
				{
					if (primul==0) primul = document.searchFormModule.cbuserlistaModule[i].value;
					lista += document.searchFormModule.cbuserlistaModule[i].value + ',';
				}
		if (lista=='')	{	alert('<?php echo JText::_($configuration->have_to_select_at_least_one_CB_user); ?>');	return;	}
		document.getElementById('save_ignore_users_action_module').value = 'save';
		document.getElementById('save_ignore_users_values_module').value = lista;
...
function ignore_selected_users_module()
	{
		...
		if (lista=='')	{	alert('<?php echo JText::_($configuration->have_to_select_at_least_one_CB_user); ?>');	return;	}
		document.getElementById('save_ignore_users_action_module').value = 'ignore';
		document.getElementById('save_ignore_users_values_module').value = lista;
		document.forms['searchFormModule'].submit();
	}
</script>
	...
	<?php if (!empty($description)) { ?>
		...
		<tr>
			<td nowrap="nowrap">
<form name="searchFormModule" id="searchFormModule" action="<?php echo $uri->toString(); ?>" method="post">
		...
		<tr>
			...
			<td width="150px" nowrap="nowrap">
				<input type="checkbox" name="checkentirelist" id="checkentirelist" value="yes"
                    onClick="javascript: if (document.searchFormModule.checkentirelist.checked==false)
							{	//	document.searchFormModule.checkentirelist.checked = true;
						for (i=0; i<document.searchFormModule.cbuserlistaModule.length; i++) document.searchFormModule.cbuserlistaModule[i].checked = false;	}
							else	{
					//	document.searchFormModule.checkentirelist.checked = false;
						for (i=0; i<document.searchFormModule.cbuserlistaModule.length; i++) document.searchFormModule.cbuserlistaModule[i].checked = true;	}">
