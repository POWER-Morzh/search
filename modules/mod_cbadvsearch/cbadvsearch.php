 ...
 *
 */
class modCbadvsearchModelCbadvsearch
{
	/**
	 * Gets the fields
	 * @return object
   ...
   */
	...
	public static function getFields($search = 1)
	{
		...
		$db->setQuery( "SELECT a.*, b.name as field_name, b.type as field_type 
			FROM #__cbadvsearch a JOIN #__comprofiler_fields b on a.field_id=b.fieldid where a.published='1' and 
			a.thesearch='".$search."' order by a.ordering" );
		$Obj = $db->loadObjectList();
		$n = count($Obj);
		for($i=0; $i<$n; $i++)
			{
	...
	for($i=$current_page-5; $i<=$current_page+5; $i++) if ($i>0 && $i<=$total)
		{
			if ($i==$current_page) $page_list .= '<span id="relative_left_padding5" style="font-weight: normal; width: 15px;">'.$i.'</span>';
				else $page_list .= '<a id="relative_left_padding5" href="javascript: setPage('.$i.');" style="font-weight: bold; width: 15px;">'.$i."</a>";
		}
	...
	function getTheSearch($search = 1)
	{
	...
	//	$this->assignRef('listing', $fieldsconfig[0]->listing);
		...
		$object[] = array("searchTotalResult" => count($searchTotalResult));
