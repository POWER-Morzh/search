 ...
 *
 */
class modCbadvsearchModelCbadvsearchResults
{
	/**
	 * Gets the fields
	 * @return object
   ...
   */
	...
	//	$this->assignRef('listing', $fieldsconfig[0]->listing);
		...
		for ($i=0, $n=count($fields); $i < $n; $i++)
		{
			...
			if (empty($_POST)) foreach($default_form_values as $dfv)
			...
			foreach($_POST as $key => $value)
				if (!empty($value))
				{
					if(!in_array($key, $words))
						{
						//	$key1 = $key;
						//	$key2 = substr($key, 0, strpos($key, "dmx2"));
						//	$key = !strpos($key, "dmx2") ? $key : $key2;
