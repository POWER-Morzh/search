...
* @version $Id: toolbar.comprofiler.html.php 1854 2012-10-01 18:03:21Z beat $
...
class cbMenuBarBase {
	...
	*/
    static function startTable() {
		if ( checkJversion( 'j3.0+' ) ) {
			echo '<div class="cbtoolbaractions">';
		} else {
			echo '<div class="cbtoolbarbar cbtoolbaractions">';
		}
    }
    static function _output( $onClick, $icon, $alt, $link = '#' ) {
    	$translated	=	CBTxt::T( $alt );
		if ( checkJversion( 'j3.0+' ) ) {
			$html	=	'<div class="btn-group" id="toolbar-' . $icon . '">'
						// .	'<a href="' . $link . '"'
					.	'<button'
						// .		( $onClick ? ' onclick="' . $onClick . '" ' : '' )
					.	( ( ! $onClick ) && $link[0] == '#' ? ' value="' . htmlspecialchars( $link ) . '"' : ' onclick="' . ( $onClick ? $onClick : 'location.href=\'' . addslashes( $link ) . '\'' ) . '"' )
					.		' class="btn btn-small cbtoolbaraction"'
					.		' title="' . htmlspecialchars( $translated ) . '">'
					.	'<i class="icon-' . $icon . '"></i> '
			...
			$html	=	'<a href="' . $link . '"'
					.		( $onClick ? ' onclick="' . $onClick . '" ' : '' )
					.		' class="cbtoolbar cbtoolbaraction">'
	...
	*/
	...
	/**
	 * Quick function for CB 1.9.1 help button
	 *
	 * @param string $ref
	 * @param bool   $com
	 */
	static function cbHelp( $ref='help', $com=false ) {
		if ( checkJversion() >= 2 ) {
			JToolbarHelper::help( $ref, $com, 'http://www.joomlapolis.com/cb-help-1-9-1');
		}
	}
...
class TOOLBAR_usersextras {
	...
	*/
	...
	static function _DEFAULT_TOOLS() {
		CBtoolmenuBar::startTable();
		CBtoolmenuBar::cbHelp();
		CBtoolmenuBar::spacer();
		CBtoolmenuBar::endTable();
