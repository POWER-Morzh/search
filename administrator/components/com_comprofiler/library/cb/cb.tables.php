...
//					$result							=	$_CB_database->query();
	...
	*/
	...
	function setRandomPassword( ) {
		if ( checkJversion() >= 2 ) {
			...
			$this->password		=	JUserHelper::genRandomPassword( 8 );
		} else {
	...
	*/
	function _cbHashPassword( $passwd, $check ) {
		...
		if ( $version >= 2 ) {
			// Use Joomla API to encrypt the password:
			$method								=	'md5-hex';
			$saltLength							=	( checkJversion( '3.2+' ) ? 16 : 32 );
			$addSalt							=	true;

			// Lets import the plugin helper if for some reason it hasn't been imported yet:
			jimport( 'joomla.plugin.helper' );

			$joomlaPluginEnabled				=	JPluginHelper::isEnabled( 'user', 'joomla' );

			// Check if encryption method can be changed via user plugin, but make sure plugin is enabled, its class actually exists, and it has the needed encryption function:
			if ( checkJversion( '3.2+' ) ) {
				...
				if ( $joomlaPluginEnabled ) {
					...
					$method						=	PlgUserJoomla::setDefaultEncryption( $userPluginParams );
			...
			// Lets import the user helper if for some reason it hasn't been imported yet:
			jimport( 'joomla.user.helper' );

			// Check if current password is a legacy password; if it is update it:
			if ( $check ) {
				$parts							=	explode( ':', $this->password );
				$updatePwd						=	false;

				if ( count( $parts ) > 1 ) {
					// We need the existing salt instead of a new one so we can match valid existing md5salted passwords if we don't update:
					$salt						=	$parts[1];

					// We only need to check to update if method is not md5salted as legacy can only be md5salted in this case:
					if ( $method ) {
						// Check if encrypted as md5salted and method differs from md5salted:
						if ( $method != 'md5-hex' ) {
							if ( ( md5( $passwd . $salt ) . ':' . $salt ) === $this->password ) {
								// We're md5salted, but the method differs; lets update:
								$updatePwd		=	true;
							}
						}

						// Check if encrypted as bcrypt and method differs from bcrypt:
						if ( $method != 'bcrypt' ) {
							if ( ( substr( $this->password, 0, 4 ) == '$2y$' ) && $strongPasswords ) {
								// We can't compare easily so lets just see if it validates:
								if ( password_verify( $passwd, substr( $this->password, 0, 60 ) ) ) {
									// We're bcrypt, but the method differs; lets update:
									$updatePwd	=	true;
								}
							}

							if ( ( hash( 'sha256', $passwd . $salt ) . ':' . $salt ) === $this->password ) {
								// We're bcrypt sha256 fallback encoded, but method differs; lets update:
								$updatePwd		=	true;
							}
						}
					}
				} else {
					// We need an up to date salt encase there is nothing to update:
					$salt						=	JUserHelper::genRandomPassword( $saltLength );

					// If password is md5 lets update it to Joomlas method:
					if ( md5( $passwd ) === $this->password ) {
						$updatePwd				=	true;
					}

					// Check if encrypted as bcrypt and method differs from bcrypt (we need this here because bcrypt can be used without a salt):
					if ( $method != 'bcrypt' ) {
						if ( ( substr( $this->password, 0, 4 ) == '$2y$' ) && $strongPasswords ) {
							// We can't compare easily so lets just see if it validates:
							if ( password_verify( $passwd, substr( $this->password, 0, 60 ) ) ) {
				...
				if ( $updatePwd ) {
					// Get a new up to date salt for the password update:
					$salt						=	JUserHelper::genRandomPassword( $saltLength );
					$crypt						=	JUserHelper::getCryptedPassword( $passwd, ( $addSalt ? $salt : null ), $method );

					// Check if salt was already added; if it was lets not add it below:
					if ( explode( ':', $crypt ) > 1 ) {
						$hashedPwd				=	$crypt;
					} else {
						$hashedPwd				=	( $crypt . ( $addSalt ? ':' . $salt : null ) );
					}

					$this->password				=	$hashedPwd;

					$query						=	'UPDATE ' . $_CB_database->NameQuote( '#__users' )
												.	"\n SET " . $_CB_database->NameQuote( 'password' ) . " = " . $_CB_database->Quote( $this->password )
												.	"\n WHERE " . $_CB_database->NameQuote( 'id' ) . " = " . (int) $this->id;
					$_CB_database->setQuery( $query );
					$_CB_database->query();
				}
			} else {
				// Always use an up to date salt if we're not validating existing password:
				$salt							=	JUserHelper::genRandomPassword( $saltLength );
			}
			...
			// Legacy usage for J1.5, J1.0, and Mambo:
			$method								=	'md5';
			$saltLength							=	8;

			if ( $version == 0 ) {
				if ( function_exists( 'josHashPassword' ) ) {
					// 1.0.13+ (in fact RC3+):
					$method						=	'md5salt';
					$saltLength					=	16;
				}
			...
			switch ( $method ) {
				case 'md5salt':
					if ( $check ) {
						...
						if ( count( $parts ) > 1 ) {
							// We're already salted so we don't need to update:
							$salt				=	$parts[1];
						} else {
							$salt				=	cbMakeRandomString( $saltLength );

							// If password matches md5 and not salted lets update it:
							if ( md5( $passwd ) === $this->password ) {
				...
				default:
					if ( $check ) {
						...
						if ( count( $parts ) > 1 ) {
							...
							// If password is md5salted lets update it to md5:
		...
		if ( $check ) {
			if ( $this->password ) {
				if ( ( substr( $this->password, 0, 4 ) == '$2y$' ) && $strongPasswords ) {
					// We can't validate bcrypt so let Joomla do it if the function exists:
					return password_verify( $passwd, substr( $this->password, 0, 60 ) );
				} else {
					// Remove any prefixing as we don't generate it and it'd always cause validation to fail:
					return ( preg_replace( '/{\w+}/i', '', $hashedPwd ) === preg_replace( '/{\w+}/i', '', $this->password ) );
				}
			} else {
				// Let Joomla handle the validation:
				return true;
			}
		} else {
			return $hashedPwd;
