...
* @version $Id: plugin.foundation.php 1899 2012-11-01 14:19:23Z beat $
...
$ueConfig['version']		=	'1.9.1';			// IMPORTANT: when changing version here also change in the 6 XML installation files and in cb.installer.php
 ...
 *
 * @param  string  $info  'api', 'product', 'release', 'j3.0+'
...
function checkJversion( $info = 'api' ) {
	...
	switch ( $info ) {
		...
		case 'j3.0+':
			$cms_version				=	substr( $VO->RELEASE, 0, 3 );
			$version[$info]				=	( $VO->PRODUCT == 'Joomla!' ) && ( strcasecmp( $cms_version, '3.0' ) >= 0 );
			break;
		default:
...
class CBTxt {
	...
	*/
	...
	public static function prepareHtmlContentPlugins( $htmlText, $allowed = true ) {
		if ( $allowed && is_callable( array( 'JHtml', '_' ) ) ) {

			// Work-around a bug in Joomla: We always want HTML from those guys!, also with format=raw used for IPNs:
			$docTypeSave		=	JFactory::getDocument()->getType();
			JFactory::getDocument()->setType( 'html' );
			jimport('joomla.application.module.helper');

			// Now fire the content plugins in a fail-safe way:
			try {
				$htmlText		=	 JHtml::_( 'content.prepare', $htmlText );
			} catch (Exception $e) {
				trigger_error('ERROR IN A CONTENT PLUGIN: ' . (string) $e , E_USER_WARNING );
			}
			JFactory::getDocument()->setType( $docTypeSave );
		}
		return $htmlText;
	}
	/**
	...
	public static function replaceUserVars( $mainText, $user_id, $html, $translateMainText = true, $prepareHtmlContentPlugins = false, $extraHtmlStrings = null, $extraNonHtmlStrings = null ) {
		if ( $translateMainText ) {
			$mainText		=	$html ? self::Th( $mainText ) : self::T( $mainText );
		}
		if ( $prepareHtmlContentPlugins ) {
			$mainText		=	self::prepareHtmlContentPlugins( $mainText );
			if ( ! $html ) {
				$mainText	=	strip_tags( $mainText );
			}
		}
		$mainText			=	CBuser::getInstance( $user_id )->replaceUserVars( $mainText, true, false, $extraNonHtmlStrings, false );
		if ( $extraHtmlStrings ) {
