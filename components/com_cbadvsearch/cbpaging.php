 ...
 * @since		1.5
 */
class JPaginationadv extends JObject
{
	/**
	...
	function getPagesCounter()
	{
		...
		if ($this->get('pages.total') > 1) {
			$html .= JText::sprintf('Page %d of %d', $this->get('pages.current'), $this->get('pages.total'));
		}
		return $html;
	}
	...
	function _item_active(&$item)
	{
		...
		if ($mainframe->isAdmin())
		{
			...
			if($item->base>0)
				return "<a title=\"".$item->text."\" href=\"javascript:void(0);\" onclick=\"javascript: document.searchForm.limitstart.value=".$item->base."; document.searchForm.submit();return false;\">".$item->text."</a>";
			else
				return "<a title=\"".$item->text."\" href=\"javascript:void(0);\" onclick=\"javascript: document.searchForm.limitstart.value=0; document.searchForm.submit();return false;\">".$item->text."</a>";
		}
	}
